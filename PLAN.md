...
- [x] **Phase 1: Restore hosting, and verify every load-bearing assumption**
      ...
      - [x] Create a **new** Vercel project pointed at the fork. **Done 24 August 2026** — project `dbas-web` under the `dbas1` (DBAS) team, live at **https://dbas-web-tau.vercel.app**, deploying from `main`. Both environment variables are set. Not attached to the club's existing Vercel project. **The URL is needed at build time, not just at runtime** — `next.config.ts` derives the allowed `next/image` remote host from it, so a build without it rejects every Storage photo on `/about` as well as serving fallback content; the deployed build has it, confirmed by a Storage image loading on `/events`.
            **Do not guess this hostname.** Vercel did not assign the conventional `dbas-web-dbas1.vercel.app`; it assigned a `-tau` suffix. The `-dbas1` host also exists, is an internal alias, and sits behind Vercel Authentication — so probing it returns a `302` to `vercel.com/sso-api` and looks exactly like a site that has been locked down. Read the production URL off `vercel project ls` rather than inferring it.
